...
class _product_sentiment_analysis:
    ...
    def getData(self, filename, start_time, end_time, num):
        ...
        outputFile = codecs.open('static/tmp/{}'.format(filename), "w+", "utf-8")
        #print(len(tweets))
        for t in tweets:
            tmp = {}
    ...
    def cluster_data(self, filename):
        ...
        dataset = list()
        with open('static/tmp/{}'.format(filename)) as data:
            for line in data:
        ...
        for x in range(0, true_k):
            if x not in noise_clusters:
                ...
                with open('static/tmp/clusters/cluster{0}.txt'.format(x), 'w') as fw:
                    for i in index:
                        fw.write(documents[i])
                        fw.write('\n')

    def get_sentiment(self):
        path = 'static/tmp/clusters/'
        upper = 0
        totalN = 0
    ...
    def get_last_week(self):
        ...
        for i in range(7):
            ...
            self.getData('tmp.json', start_time.isoformat(), end_time.isoformat(), 1000)#5000)
    ...
    def get_last_month(self):
        ...
        for i in range(30):
            end_time = start_time + one_day
            self.getData('tmp.json', start_time.isoformat(), end_time.isoformat(), 1000)
            self.cluster_data('tmp.json')
            score = self.get_sentiment()
            x.insert(0, start_time.isoformat())
            y.insert(0, score)
    ...
    def get_recent(self, interval, length):
        days = timedelta(int(interval))
        start_time = self.date - days
        x = list()
        y = list()
        for i in range(int(length)):
            end_time = start_time + days
            self.getData('tmp.json', start_time.isoformat(), end_time.isoformat(), 100)
            self.cluster_data('tmp.json')
            score = self.get_sentiment()
